...
function A_out = change_MPO_local(A_in, isite, y)
    ...
    % reshape(A_in{isite}, [], d): Returns A_in{isite} as an array of d
    % columns. Each column is the (chi x chi) matrix (1 x chiif isite~=1,N)
    % written as a vector.
    % Short story: Reshape Ain in M^2 columns
