function plotMPOdata_2D(source,tmin,tlim,coloresid,cases, input )
...
switch coloresid
   ...
   case 9
     ...
     fig1=figure(1);clf(1);figure(1) % dynamics
     fig2=figure(2);clf(2);figure(2) % contour populations
     fig3=figure(3);clf(3);figure(3) % coherences rho_12
     %´fig4=figure(4);clf(4);figure(4) % vibrational states
     %fig5=figure(5);clf(5);figure(5) % vibrational states
     %fig6=figure(6);clf(6);figure(6) % vibrational enrgy
     %fig7=figure(7);clf(7);figure(7) % bond dimension
     fig8=figure(8);clf(8);figure(8) % computation time
    ...
    for qq=1:numel(source)
            ...
            lattice = source{qq}.lattice;
            latticep1 = lattice+1;
            lattpos = source{qq}.lattpos;
            lattdis = source{qq}.lattdis;            
            ...
            % Distance (after normalization)
                ...
                distance_1d = zeros(nmax,1);
                for tt=1:nmax                                                  
                    for ii=2:N                                  
                      distance(tt)= distance(tt) + lattdis( latticep1 == ii ) * rho_out(tt,pop_idx(ii));                                                      
                    end
                    for jj=1:size( lattice, 2 )
                        distance_1d(tt)= distance_1d(tt) + jj * sum( rho_out(tt, pop_idx( latticep1(:,jj) )), 2 );
                    end
            ...
            figure(1)
                ...
                plot(time,distance_1d',colores(qq),'LineStyle','--');hold on                    
                ...
                subplot(2,3,3)                                           
                %plot(time, sum( rho_out(:,pop_idx(7:end)),2 ) ,colores(qq),'LineStyle','-','LineWidth',2);hold on
                plot(time, sum( rho_out(:,pop_idx([1 2])),2),colores(qq),'LineStyle','--','LineWidth',1); hold on                    
               % plot(time, sum( rho_out(:,pop_idx( latticep1(:,[4,5]) ) ) ,2),colores(qq),'LineStyle','-','LineWidth',1); hold on                  
                grid on
           ...
           figure(3)                      
                subplot(211)
                plot(time,imag(rho_out(:,5)),colores(qq) ), hold on 
                xlim([0,tlim])       
                ylim([-0.1 0.1])                         
                ...
                subplot(212)
                plot(time,imag(rho_out(:,8)),colores(qq), 'LineStyle','-'), hold on             
            ...
            if vibrations_info    
                ...
                for oo=1:numel(hfidx) 
                        ...
                        % Vibrational energy                                                                                               
                        vibengy(:,jij) =  abs( rhoscs{ hfidx(oo) }(:,n_idx)) * [0:1500:(Nb(whichosc)-1)*1500]';                                                  
                ...
                figure(6)
                        ...
                        plot(time, distance / max(distance), colores(qq) ), hold on
